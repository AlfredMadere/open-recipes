...
def test_create_recipe_list():
    ...
    response = client.post(
        "/recipe-lists",
        json={"name": "My Recipes", "description": "My favorite recipes"},
    )
    assert response.status_code == 201
    client.post(
        "/recipe-lists",
        json={"name": "My Recipes 2", "description": "My almost favorite recipes"},
    )
    assert response.status_code == 201
...
def test_create_recipe():
    ...
    response = client.post(
        ...
        json={
            ...
            "procedure": "Cook spaghetti, fry bacon, mix with eggs and cheese",
        },
    )
    assert response.status_code == 201
    ...
    client.post(
        ...
        json={
            ...
            "procedure": "Cook spaghetti, fry bacon, mix with eggs and cheese but do it twice",
        },
    )
    assert response.status_code == 201
...
def test_create_tag():
    ...
    # Create a tag
    response = client.post(
        f'/recipes/{recipe_id}/tags/{tag_create_response1.json()["id"]}'
    )
    assert response.status_code == 201
...
def test_filter_tag():
    ...
    # Create tags
    client.post(
        f'/recipes/{response1.json()["id"]}/tags/{tag_create_response1.json()["id"]}',
        json=tag_1_popo,
    )
    client.post(
        f'/recipes/{response2.json()["id"]}/tags/{tag_create_response2.json()["id"]}',
        json=tag_2_popo,
    )
    client.post(
        f'/recipes/{response3.json()["id"]}/tags/{tag_create_response1.json()["id"]}',
        json=tag_1_popo,
    )
    client.post(
        f'/recipes/{response4.json()["id"]}/tags/{tag_create_response2.json()["id"]}',
        json=tag_2_popo,
    )
...
def test_create_ingredient():
    ...
    user_response = client.post(
        "/users", json={"name": "John Doe", "email": "john@doe.com"}
    )
    user_id = user_response.json()["id"]

    response = client.post(
        "/ingredients", json={"name": "Spaghetti", "type": "pasta", "storage": "PANTRY"}
    )
    ...
    response = client.post(
        "/ingredients",
        json={"name": "other_ingredient", "type": "pasta", "storage": "FRIDGE"},
    )
    assert response.status_code == 201
    # associcate ingredient with user
    response = client.post(f'/users/{user_id}/ingredients/{response.json()["id"]}')
...
def test_user_inventory_search():
    # create ingreadient with user
    user_response = client.post(
        "/users", json={"name": "John Doe", "email": "john@dioe.com"}
    )
    ...
    response2 = client.post(
        "/ingredients", json={"name": "Bacon", "type": "meat", "storage": "FRIDGE"}
    )
    response3 = client.post(
        "/ingredients", json={"name": "Eggs", "type": "dairy", "storage": "FRIDGE"}
    )
    response4 = client.post(
        "/ingredients", json={"name": "Parmesan", "type": "dairy", "storage": "FRIDGE"}
    )
    ...
    response5 = client.post(
        "/ingredients", json={"name": "Noodles", "type": "noodles", "storage": "PANTRY"}
    )
    response6 = client.post(
        "/ingredients", json={"name": "Beef", "type": "meat", "storage": "FRIDGE"}
    )
    response7 = client.post(
        "/ingredients", json={"name": "Cinnamon", "type": "spice", "storage": "PANTRY"}
    )
    ...
    response = client.post(
        f'/recipes/{recipe_id1}/ingredients/{response1.json()["id"]}'
    )
    assert response.status_code == 201
    response = client.post(
        f'/recipes/{recipe_id1}/ingredients/{response2.json()["id"]}'
    )
    assert response.status_code == 201
    response = client.post(
        f'/recipes/{recipe_id1}/ingredients/{response3.json()["id"]}'
    )
    assert response.status_code == 201
    response = client.post(
        f'/recipes/{recipe_id1}/ingredients/{response4.json()["id"]}'
    )
    assert response.status_code == 201

    response = client.post(
        f'/recipes/{recipe_id2}/ingredients/{response5.json()["id"]}'
    )
    assert response.status_code == 201
    response = client.post(
        f'/recipes/{recipe_id2}/ingredients/{response6.json()["id"]}'
    )
    assert response.status_code == 201
    response = client.post(
        f'/recipes/{recipe_id2}/ingredients/{response7.json()["id"]}'
    )
    assert response.status_code == 201
...
def test_get_ingredients_from_recipe():
    ...
    response2 = client.post(
        "/recipes",
        json={"name": "Penne and Butter", "procedure": "Cook penne, add butter"},
    )
    ...
    response3 = client.post(
        "/ingredients", json={"name": "Penne", "type": "pasta", "storage": "PANTRY"}
    )
    response4 = client.post(
        "/ingredients", json={"name": "Butter", "type": "dairy", "storage": "FRIDGE"}
    )
    response5 = client.post(
        "/ingredients",
        json={"name": "frozen Bacon", "type": "meat", "storage": "FRIDGE"},
    )
    assert response3.status_code == 201
    assert response4.status_code == 201
    assert response5.status_code == 201
    ...
    response = client.post(
        f'/recipes/{recipe_id2}/ingredients/{response3.json()["id"]}'
    )
    assert response.status_code == 201
    response = client.post(
        f'/recipes/{recipe_id2}/ingredients/{response4.json()["id"]}'
    )
    assert response.status_code == 201
    response = client.post(
        f'/recipes/{recipe_id2}/ingredients/{response5.json()["id"]}'
...
def test_flow_2():
    # Create a user
    response = client.post(
        "/users", json={"name": "Bob Sandler", "email": "bob@sandler.com"}
    )
    assert response.status_code == 201
...
def test_create_recipe2():
    response = client.post(
        ...
        json={
            ...
            "mins_prep": 20,
            "mins_cook": 30,
            "tags": [{"key": "Cuisine", "value": "Italian"}],
        },
    )
    assert response.status_code == 201
