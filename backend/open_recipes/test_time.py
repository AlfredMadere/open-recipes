...
def get_user_test(username: str):
    ...
    with engine.connect() as conn:
        result = conn.execute(
            text(
                """SELECT email, phone, name, id, hashed_password FROM "user" WHERE email = :username"""
            ),
            {"username": username},
        )
        ...
        if user_record:
            ...
            return UserInDB(
                email=email, phone=phone, name=name, hashed_password="gvgyvgy", id=id
            )
        return None
...
def time_endpoint(method, url, data):
    ...
    if method == "get":
        response = client.get(
            ...
            params=data,
            headers={
                "Bearer": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJqdWxpZWJyaWdodDFAZXhhbXBsZS5uZXQiLCJleHAiOjE3MDE3MDc4Mzl9.Ytz4irnOXO2mvJFN5ME7RZrrRbjAoBUIMrkQQvJlkX4"
            },
        )
    elif method == "post":
    ...
    assert response.status_code in [
        ...
        201,
        204,
    ]  # Update as per your API's expected responses
...
def test_endpoint_performance():
    ...
    endpoints = [
        ...
        (
            "post",
            "/users",
            {"json": {"name": "example", "email": str(uuid.uuid4()), "phone": "sdc"}},
        ),
        ...
        ("get", "/tags", {}),
        ("post", "/tags", {"json": {"key": str(uuid.uuid4()), "value": "example"}}),
        ...
        (
            "post",
            "/recipe-lists",
            {"json": {"name": str(uuid.uuid4()), "description": "example"}},
        ),
        (
            "post",
            "/recipe-lists/{recipe_list_id}/recipe/{recipe_id}",
            {
                "recipe_list_id": random.randint(1, 10000),
                "recipe_id": random.randint(1, 10000),
            },
        ),
