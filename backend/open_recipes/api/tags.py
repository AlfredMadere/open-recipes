...
def get_tags(
    ...
    try:
        stmt = sqlalchemy.select(recipe_tag.c.id, recipe_tag.c.key, recipe_tag.c.value)
        if key is not None:
    ...
    try:
        ...
        search_result = SearchResults(
            prev_cursor=prev_cursor, next_cursor=next_cursor, tags=tags_result
        )
    except exc.SQLAlchemyError as e:
        raise HTTPException(status_code=500, detail="Database error " + e._message())
...
@router.post(
    "", response_model=None, status_code=201, responses={"201": {"model": Tag}}
)
def create_tag(
    tag: CreateTagRequest, engine: Annotated[Engine, Depends(get_engine)]
) -> Union[None, Tag]:
    try:
        with engine.begin() as conn:
            result = conn.execute(
                text(
                    """INSERT INTO recipe_tag (key, value) VALUES (:key, :value) RETURNING id, key, value"""
                ),
...
def get_tag_by_id(tag_id: int, engine: Annotated[Engine, Depends(get_engine)]) -> Tag:
    try:
        with engine.begin() as conn:
            result = conn.execute(
                text("""SELECT id, key, value FROM "recipe_tag" WHERE id = :id"""),
                {"id": tag_id},
            )
            row = result.fetchone()
            if row is None:
