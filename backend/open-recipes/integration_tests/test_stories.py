...
def test_create_recipe_list():
    ...
    response = requests.post('http://localhost:8000/recipe-lists', json={'name': 'My Recipes', 'description': 'My favorite recipes'})
    assert response.status_code == 201

    # Get the recipe list ID from the response
    recipe_list_id = response.json()['id']
    test_entities.add_recipe_list(recipe_list_id)

    # Send a GET request to verify the recipe list was created
    response = requests.get(f'http://localhost:8000/recipe-lists/{recipe_list_id}')
    assert response.status_code == 200
...
def test_create_recipe():
    ...
    response = requests.post('http://localhost:8000/recipes', json={'name': 'Spaghetti Carbonara', 'instructions': 'Cook spaghetti, fry bacon, mix with eggs and cheese'})
    assert response.status_code == 201
    ...
    response = requests.get(f'http://localhost:8000/recipes/{recipe_id}')
    assert response.status_code == 200
...
#     response = requests.post('http://localhost:8000/recipe-lists', json={'name': 'My Recipes'})
#     recipe_list_id = response.json()['id']

#     # Add the recipe to the recipe list
#     response = requests.post(f'http://localhost:8000/recipes/{recipe_id}/recipe-lists/{recipe_list_id}')
#     assert response.status_code == 201
