...
def generate_recipe_lists(n):
    ...
    for i in range(1, n + 1):
        recipe_lists.append(
            {"id": i, "name": fake.word(), "description": fake.text(max_nb_chars=200)}
        )
    return recipe_lists
...
def generate_recipe_tags(n):
    recipe_tags = []
    for i in range(1, n + 1):
        recipe_tags.append({"id": i, "key": fake.word(), "value": fake.word()})
    return recipe_tags
...
def generate_recipe_x_tags(n, recipe_ids, tag_ids):
    ...
    for _ in range(n):
        recipe_x_tags.append(
            {"recipe_id": random.choice(recipe_ids), "tag_id": random.choice(tag_ids)}
        )
...
recipes = generate_recipes(
    num_recipes, [u["id"] for u in users], [rl["id"] for rl in recipe_lists]
)
reviews = generate_reviews(
    num_reviews, [u["id"] for u in users], [r["id"] for r in recipes]
)
...
recipe_ingredients = generate_recipe_ingredients(
    num_recipe_ingredients,
    [ing["id"] for ing in ingredients],
    [r["id"] for r in recipes],
)
recipe_tags = generate_recipe_tags(num_recipe_tags)
recipe_x_tags = generate_recipe_x_tags(
    num_recipe_x__tag, [r["id"] for r in recipes], [rt["id"] for rt in recipe_tags]
)
user_x_recipe_lists = generate_user_x_recipe_lists(
    num_user_x_recipe_lists, [u["id"] for u in users], [rl["id"] for rl in recipe_lists]
)
user_x_ingredients = generate_uver_x_ingredients(
    num_user_x_ingredients, [u["id"] for u in users], [ing["id"] for ing in ingredients]
)
...
def insert_recipe_x_recipe_lists():
    with get_engine().connect() as conn:
        query_str = (
            "INSERT INTO recipe_x_recipe_list (recipe_id, recipe_list_id) VALUES "
        )
...
def insert_all():
    ...
    with get_engine().connect() as conn:
        ...
        result = conn.execute(
            text("SELECT pg_get_serial_sequence('recipe_list', 'id')")
        )
        conn.execute(text(f"ALTER SEQUENCE {result.fetchone()[0]} RESTART WITH 1"))
        ...
        result = conn.execute(
            text("SELECT pg_get_serial_sequence('ing_category', 'id')")
        )
        conn.execute(text(f"ALTER SEQUENCE {result.fetchone()[0]} RESTART WITH 1"))
